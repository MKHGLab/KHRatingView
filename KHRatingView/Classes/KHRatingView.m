...
#import "KHRatingView.h"
#import <QuartzCore/QuartzCore.h>

@implementation KHRatingView {
...
- (void)KHRatingViewInit {
    _ratingViewTag = 0;
  _cachedMarkCharacterSize = CGSizeZero;
  _markCharacter = @"\u2605";
    _starSize = 22;
  _markFont = [UIFont systemFontOfSize:_starSize];
   // _starSize = [UIFont systemFontOfSize:30.0];
...
- (void)setRatingViewTag:(int)ratingViewTag{
    if(_ratingViewTag != ratingViewTag){
        _ratingViewTag = ratingViewTag;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(CGRectMake(frameOffsetX - _markImage.size.width, frameOffsetY, totalStartsWidth + _markImage.size.width, _markImage.size.height), location)) {
        ...
        [self.delegate ratingChanged:_ratingViewTag ratingValue:value];
    }
}
